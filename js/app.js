...
function handleFreeSlotClick(e){
  ...
  if(state.calendarSelectionMode){const mode=state.calendarSelectionMode;state.calendarSelectionMode=null;if(mode==='personal'){openPersonalForm(null,key,time);}else{openNewAppointmentForm();}return;}
...
function openDay(key, selectable=false) {
  ...
  if(selectable)document.querySelector('#selectThisDay')?.addEventListener('click',()=>{const mode=state.calendarSelectionMode;state.selectedDateKey=key;state.modal=null;state.calendarSelectionMode=null;if(mode==='personal')openPersonalForm(null,key);else openNewAppointmentForm();});
}
...
async function shareRulesAndText(a){const c=currentData().clients.find(x=>x.id===a.clientId),text=reservationText(a,false);try{const res=await fetch(BOOKING_RULES_IMAGE),blob=await res.blob(),file=new File([blob],'informacoes-agendamento.jpg',{type:blob.type||'image/jpeg'});if(navigator.canShare?.({files:[file]})){await navigator.share({files:[file],text,title:'Informações para agendamento'});markMessageSent(a,'reservation_full');return;}}catch(e){}await shareBookingRulesImage();setTimeout(()=>openWhatsApp(c?.whatsapp,text),400);markMessageSent(a,'reservation_full');}
