...
async function shareRulesAndText(a){const c=currentData().clients.find(x=>x.id===a.clientId),text=reservationText(a,false);openWhatsApp(c?.whatsapp,text);markMessageSent(a,'reservation_full');setTimeout(()=>{if(confirm('A conversa da cliente foi aberta com as regras e o sinal. Deseja abrir também a imagem das regras para anexar no WhatsApp?'))window.open(BOOKING_RULES_IMAGE,'_blank');},900);}
