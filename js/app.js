...
function finance() {
  ...
  return `<main class="content"><section class="hero-card"><span class="eyebrow">FINANCEIRO</span><h1>Entradas e gastos</h1><p>${weekLabel(state.selectedWeek)}</p><div class="week-controls finance-week"><button id="prevWeek">‹</button><button id="thisWeek">Esta semana</button><button id="nextWeek">›</button></div></section>
    ...
    <section class="section-card"><div class="section-head"><div><span class="eyebrow">RELATÓRIO GERAL</span><h2>Acompanhamento visual</h2><p>${rr.label}</p></div></div><div class="dashboard-tabs"><button data-fin-report="week" class="${state.financeReportMode==='week'?'active':''}">Semana</button><button data-fin-report="month" class="${state.financeReportMode==='month'?'active':''}">Mês</button><button data-fin-report="year" class="${state.financeReportMode==='year'?'active':''}">Ano</button></div><label>Selecionar período<input id="financeReportDate" type="date" value="${toLocalDateKey(new Date(state.financeReportAnchor))}"></label><section class="metric-grid"><button class="metric metric-button" id="reportIncomeCard"><span>Entradas</span><strong>${brl(reportIncome)}</strong><small>Ver procedimentos</small></button><article class="metric"><span>Saídas</span><strong>${brl(reportExpenses)}</strong></article><article class="metric"><span>Resultado</span><strong>${brl(reportIncome-reportExpenses)}</strong></article><article class="metric"><span>Atendimentos</span><strong>${completed.length}</strong></article><article class="metric"><span>Cancelamentos</span><strong>${cancelled.length}</strong></article></section>${walletIncome||walletExpense?`<div class="note-box">Carteira lançada no financeiro neste período: <b>${brl(walletIncome)}</b> em entradas e <b>${brl(walletExpense)}</b> em saídas.</div>`:''}</section>
...
function openProcedureReport(){const {start,end,label}=financeReportRange(),aps=currentData().appointments.filter(a=>a.type!=='personal'&&a.status==='completed'&&new Date(a.start)>=start&&new Date(a.start)<=end),rows=new Map();for(const a of aps){const items=a.procedureItems?.length?a.procedureItems:(a.procedureIds||[]).map((id,i)=>({id,name:a.procedureNames?.[i]||currentData().procedures.find(p=>p.id===id)?.name||'Procedimento',value:Number(a.totalValue||0)/(a.procedureIds?.length||1)}));for(const i of items){const key=i.id||i.name,r=rows.get(key)||{name:i.name||currentData().procedures.find(p=>p.id===i.id)?.name||'Procedimento',qty:0,value:0};r.qty++;r.value+=Number(i.value||0);rows.set(key,r);}}const list=[...rows.values()].sort((a,b)=>b.qty-a.qty||b.value-a.value);state.modal=`<div class="modal-head"><div><span class="eyebrow">PROCEDIMENTOS REALIZADOS</span><h2>${escapeHtml(label)}</h2></div><button data-close-modal>×</button></div><div class="received-total">Atendimentos concluídos: <b>${aps.length}</b></div><div class="simple-list">${list.length?list.map(r=>`<div class="receipt-row"><div><strong>${escapeHtml(r.name)}</strong><span>${r.qty} ${r.qty===1?'procedimento':'procedimentos'} realizados</span></div><b>${brl(r.value)}</b></div>`).join(''):'<div class="empty">Nenhum procedimento concluído neste período.</div>'}</div>`;render();}
...
function bind() {
  ...
  document.querySelector('#financeReportDate')?.addEventListener('change',e=>{if(e.target.value){state.financeReportAnchor=new Date(e.target.value+'T12:00:00');render();}});
...
function openClient(id){
  ...
  document.querySelector('#clientSchedule').onclick=()=>openClientScheduleChoice(id);document.querySelector('#clientSendRules').onclick=()=>shareBookingRulesImage();document.querySelector('#clientWallet')?.addEventListener('click',()=>openClientWallet(id));document.querySelector('#enableClientWallet')?.addEventListener('click',()=>{if(confirm('Ativar saldo em carteira para esta cliente?')){c.walletEnabled=true;c.walletTransactions=c.walletTransactions||[];store.save('client.wallet.enable',{clientId:id});openClient(id);}});document.querySelectorAll('[data-appointment]').forEach(b=>b.onclick=()=>openAppointment(b.dataset.appointment));document.querySelectorAll('[data-conclude-client]').forEach(b=>b.onclick=()=>completeAppointment(b.dataset.concludeClient));
}
...
function openAppointment(id){
  ...
  state.modal=`<div class="modal-head"><div><span class="eyebrow">AGENDAMENTO${completed?' • CONCLUÍDO':cancelled?' • CANCELADO':awaiting?' • AGUARDANDO CONFIRMAÇÃO':''}</span><h2>${escapeHtml(c?.name||'Cliente')}</h2></div><button data-close-modal>×</button></div><div class="detail-grid"><span>Data</span><b>${dateWithWeekday(a.start)}</b><span>Horário</span><b>${fmtTime(a.start)}–${fmtTime(a.end)}</b><span>Procedimentos</span><b>${escapeHtml(a.procedureNames?.join(', ')||'')}</b><span>Valor</span><b>${brl(a.totalValue)}</b>${a.carryOver?.total?`<span>Inclui saldo anterior</span><b>${brl(a.carryOver.total)}</b>`:''}${a.transferredOut?`<span>Saldo transferido</span><b>${brl(a.transferredOut)}</b>`:''}<span class="received-band ${balance<=0?'received-paid':'received-pending'}">Recebido</span><b class="received-band ${balance<=0?'received-paid':'received-pending'}">${brl(paid)}</b><span>A receber</span><b>${brl(balance)}</b></div>${a.notes?`<div class="note-box">${escapeHtml(a.notes)}</div>`:''}${awaiting?`<button class="primary full" id="reservationActions">Reserva / sinal</button><button class="secondary full" id="remindConfirmation">${messageButtonLabel(a,'confirmation_reminder')} • lembrete de confirmação</button>`:''}${!cancelled?`${financeButton}${!completed?'<button class="success-action full" id="completeAppointment">✓ Concluir atendimento</button>':''}${completed&&balance>0?`<button class="primary full" id="payCompletedBalance">Registrar pagamento pendente • ${brl(balance)}</button>`:''}${completed&&needsMaintenance?'<button class="secondary full" id="scheduleMaintenance">Agendar próxima manutenção</button>':''}${completed&&care?'<button class="secondary full" id="sendPostCare">Enviar cuidados pós-atendimento</button>':''}${!completed?'<div class="action-pair"><button class="secondary" id="editSchedule">Alterar data / horário</button><button class="secondary" id="rescheduleCancel">Reagendar / cancelar</button></div>':''}${paid<=0&&!completed?'<button class="danger full" id="deleteAppointment">Excluir agendamento</button>':''}`:''}${a.history?.length?`<details class="appointment-history"><summary>Ver histórico do agendamento</summary>${a.history.slice().reverse().map(h=>`<div><b>${dateWithWeekday(h.at,true)}</b><span>${escapeHtml(friendlyHistoryLabel(h))}</span></div>`).join('')}</details>`:''}`;render();
...
async function shareBookingRulesImage(){await shareAssetWithText(BOOKING_RULES_IMAGE,'regras-agendamento-juliane-boone.jpg','image/jpeg','Regras de agendamento • Juliane Boone Nail Designer');}
...
function openClientWallet(clientId){const c=currentData().clients.find(x=>x.id===clientId);if(!c)return;const tx=walletTransactions(c).slice().sort((a,b)=>new Date(b.at)-new Date(a.at));state.modal=`<div class="modal-head"><div><span class="eyebrow">CARTEIRA DA CLIENTE</span><h2>${escapeHtml(c.name)}</h2></div><button data-close-modal>×</button></div><div class="cycle-alert"><b>Saldo atual: ${brl(walletBalance(c))}</b></div><button class="primary full" id="addWalletCredit">+ Adicionar saldo</button><button class="secondary full" id="toggleWalletOff">Desativar saldo em carteira</button><div class="simple-list">${tx.length?tx.map(t=>`<div class="receipt-row"><div><strong>${escapeHtml(t.description||'Movimentação')}</strong><span>${dateWithWeekday(t.at)}${t.appointmentId?' • atendimento':''}</span></div><b>${t.amount>=0?'+ ': '− '}${brl(Math.abs(t.amount))}<small> • saldo ${brl(t.balanceAfter??0)}</small></b></div>`).join(''):'<div class="empty">Nenhuma movimentação na carteira.</div>'}</div>`;render();document.querySelector('#addWalletCredit').onclick=()=>openWalletCreditForm(clientId);document.querySelector('#toggleWalletOff').onclick=()=>{if(confirm('Desativar saldo em carteira? O saldo e todo o histórico serão preservados.')){c.walletEnabled=false;store.save('client.wallet.disable',{clientId});openClient(clientId);}};}
