...
function clientPendingAppointments(clientId,excludeId=null,beforeStart=null){return currentData().appointments.filter(a=>a.type!=='personal'&&a.clientId===clientId&&a.id!==excludeId&&a.status!=='cancelled'&&(!beforeStart||new Date(a.start)<new Date(beforeStart))&&appointmentBalance(a)>0);}
function pendingBalanceSummary(clientId,excludeId=null,beforeStart=null){const items=clientPendingAppointments(clientId,excludeId,beforeStart);return {items,total:items.reduce((sum,a)=>sum+appointmentBalance(a),0)};}
function carryPendingIntoAppointment(target,items){if(!target||!items?.length)return 0;let total=0;const carried=[];for(const src of items){const amount=appointmentBalance(src);if(amount<=0)continue;src.transferredOut=Number(src.transferredOut||0)+amount;src.transferredTo=target.id;addHistory(src,'balance_transferred',`Saldo ${brl(amount)} transferido para outro atendimento`,{toAppointmentId:target.id});carried.push({appointmentId:src.id,amount});total+=amount;}if(total>0){target.carryOver={items:carried,total};target.totalValue=Number(target.totalValue||0)+total;addHistory(target,'balance_carried',`Saldo anterior transferido: ${brl(total)}`);}return total;}
...
function serviceValueForAppointment(a){if(!a)return 0;/* O valor financeiro real do novo serviço é o total efetivamente salvo no atendimento, descontado apenas o saldo anterior transferido. procedureItems preserva a composição/histórico, mas não deve sobrescrever um valor total ajustado manualmente. */return Math.max(0,Number(a.totalValue||0)-Number(a?.carryOver?.total||0));}
...
function appointmentRow(a) {
  ...
  const settled=paid+Number(a.transferredOut||0),total=Number(a.totalValue||0),isPaid=total>0&&settled>=total,isFinancialPending=!isPaid&&a.status!=='awaiting_confirmation';const status=a.status==='awaiting_confirmation'?'Aguardando':a.status==='completed'?(isPaid?'Realizado • Pago':settled>0?'Realizado • Parcial':'Realizado • A receber'):isPaid?'Confirmado • Pago':settled>0?'Confirmado • Parcial':'Confirmado';
  const rowClass=isPaid&&a.status==='completed'?'realized-paid-row':a.status==='awaiting_confirmation'?'awaiting-row':a.status==='scheduled'?'ongoing-row':'';
  return `<button class="appointment-row ${rowClass}" data-appointment="${a.id}"><div class="date-box"><b>${fmtDayShort(a.start).split(' ')[0]}</b><small>${fmtTime(a.start)}–${fmtTime(a.end)}</small></div><div class="appointment-main"><strong>${escapeHtml(client?.name||'Cliente')}</strong><span>${escapeHtml(a.procedureNames?.join(', ')||'Atendimento')}</span></div><div class="payment-state ${isFinancialPending?'financial-pending':''}">${status}</div><span class="chevron">›</span></button>`;
}
...
function weekCalendar() {
  ...
  const label=`<span class="agenda-title-main">${days[0].getDate()} a ${days[6].getDate()}</span><small class="agenda-title-sub">${days[0].toLocaleDateString('pt-BR',{month:'long',year:'numeric'})}</small>`;
...
function timelineBlock(a){
  ...
  const top=sm/960*100,height=Math.max(1.4,(em-sm)/960*100),cls=a.type==='personal'?'personal':a.status==='completed'?'completed':a.status==='awaiting_confirmation'?'awaiting':'service';
...
function focusBlock(a){
  ...
  const top=sm/960*100,height=Math.max(2.2,(em-sm)/960*100),label=a.type==='personal'?(a.personalKind||'Particular'):(currentData().clients.find(c=>c.id===a.clientId)?.name||'Atendimento'),statusCls=a.type==='personal'?'personal':a.status==='completed'?'completed':a.status==='awaiting_confirmation'?'awaiting':'service';
  return `<button class="focus-block ${statusCls}" data-focus-appt="${a.id}" style="top:${top}%;height:${height}%"><b>${fmtTime(a.start)}–${fmtTime(a.end)}</b><span>${escapeHtml(label)}</span></button>`;
}
...
function monthCalendar() {
  ...
  const monthLabel=`<span class="agenda-title-main capitalize">${new Intl.DateTimeFormat('pt-BR',{month:'long'}).format(d)}</span><small class="agenda-title-sub">${year}</small>`;
  return `<main class="content ${state.calendarSelectionMode?'calendar-selecting':''}"><section class="section-card calendar-card ${state.calendarSelectionMode?'selection-focus':''}"><div class="section-head"><div><span class="eyebrow">AGENDA • MÊS</span><h1 class="capitalize">${monthLabel}</h1></div><div class="week-controls"><button id="prevMonth">‹</button><button id="todayMonth">Hoje</button><button id="nextMonth">›</button></div></div><div class="agenda-view-tabs"><button data-agenda-mode="week">Semana</button><button data-agenda-mode="month" class="active">Mês</button></div>${state.calendarSelectionMode?`<div class="calendar-selection-hint"><button id="cancelCalendarSelection" class="calendar-selection-close" aria-label="Voltar">×</button><span>${state.calendarSelectionMode==='maintenance'?'Escolha o dia do retorno':'Toque no dia desejado'}</span></div>`:''}<div class="legend"><span><i class="status-dot awaiting"></i>Aguardando</span><span><i class="status-dot ongoing"></i>Em andamento</span><span><i class="status-dot completed"></i>Realizado</span><span><i class="status-dot personal"></i>Particular</span></div><div class="calendar-grid dow">${['DOM','SEG','TER','QUA','QUI','SEX','SÁB'].map(x=>`<b>${x}</b>`).join('')}</div><div class="calendar-grid">${cells.map(calCell).join('')}</div></section></main>`;
}
...
function calCell(date) {
  ...
  const hasAwaiting=service.some(a=>a.status==='awaiting_confirmation'),hasOngoing=service.some(a=>a.status==='scheduled'),hasCompleted=service.some(a=>a.status==='completed'),hasPersonal=personal.length>0;
  const statusDots=[hasAwaiting?'awaiting':null,hasOngoing?'ongoing':null,hasCompleted?'completed':null,hasPersonal?'personal':null].filter(Boolean);
  return `<button class="day-cell ${cls} ${today?'today':''} ${state.calendarSelectionMode?'awaiting-choice':''}" data-day="${key}"><span class="day-number">${date.getDate()}</span>${aps.length?`<span class="count-badge">${aps.length}</span>`:''}${statusDots.length?`<span class="month-status-dots">${statusDots.map(x=>`<i class="status-dot ${x}"></i>`).join('')}</span>`:''}</button>`;
}
...
function catalogCards(publicMode=false) {
  return currentData().procedures.filter(p=>p.active).map(p=>`<article class="portfolio-card${publicMode?' pdf-service-card':''}">
...
function catalog() {
  ...
  <section class="catalog-showcase">${catalogCards(false)}</section><div class="note-box"><b>Catálogo Oficial dinâmico</b><br>Os valores e descrições compartilhados vêm deste cadastro. Ao alterar um procedimento no ADM, o catálogo compartilhado acompanha a alteração.</div><button class="secondary full" id="previewOfficialCatalog">Visualizar Catálogo Oficial</button>
...
function bind() {
  ...
  document.querySelector('#reportIncomeCard')?.addEventListener('click',openProcedureReport);document.querySelector('#reportExpenseCard')?.addEventListener('click',openExpenseReport);document.querySelector('#reportAppointmentsCard')?.addEventListener('click',openAppointmentsReport);
...
function bindCalendarPeriodSwipe(){
  if(state.view!=='calendar'||state.expandedDayKey)return;
  const el=document.querySelector('.calendar-card');if(!el)return;let sx=0,sy=0,tracking=false;
  el.addEventListener('touchstart',e=>{if(e.touches.length!==1||e.target.closest('input,select,textarea,label'))return;sx=e.touches[0].clientX;sy=e.touches[0].clientY;tracking=true;},{passive:true});
  el.addEventListener('touchend',e=>{if(!tracking)return;tracking=false;const t=e.changedTouches[0],dx=t.clientX-sx,dy=t.clientY-sy;if(Math.abs(dx)<65||Math.abs(dx)<Math.abs(dy)*1.25)return;const dir=dx<0?1:-1;if(state.agendaMode==='month')state.selectedMonth=new Date(state.selectedMonth.getFullYear(),state.selectedMonth.getMonth()+dir,1);else{state.selectedWeek=new Date(state.selectedWeek);state.selectedWeek.setDate(state.selectedWeek.getDate()+dir*7);}render();},{passive:true});
}
...
function saveAppointment() {
  ...
  if(!state.draftAppointment?.debtChecked&&!state.draftAppointment?.carryOver){const pending=pendingBalanceSummary(clientId,null,start);if(pending.total>0){state.draftAppointment={...(state.draftAppointment||{}),clientId,debtChecked:true,formSnapshot:{clientId,procIds:[...procIds],quantities:Object.fromEntries(procIds.map(id=>[id,id==='reposicao-quebrada'?Number(document.querySelector(`[data-qty="${id}"]`)?.textContent||1):1])),extras:extraRows.map(row=>({description:row.querySelector('[data-extra-description]')?.value.trim()||'',value:row.querySelector('[data-extra-value]')?.value||''})),start:startTime,end:endTime,value:document.querySelector('#apptValue').value,notes:document.querySelector('#apptNotes').value,newApplication:!!document.querySelector('#newApplicationChoice')?.checked}};state.modal=`<div class="modal-head"><div><span class="eyebrow">SALDO PENDENTE</span><h2>Antes de confirmar</h2></div><button data-close-modal>×</button></div><div class="warning">Esta cliente possui <b>${brl(pending.total)}</b> a receber de atendimento anterior.</div><p class="muted">O novo agendamento pode continuar. Na conclusão, o sistema perguntará se deseja adicionar esse saldo ao atendimento atual.</p><button class="primary full" id="continuePendingSchedule">Continuar agendamento</button><button class="secondary full" id="backPendingSchedule">Voltar</button>`;render();document.querySelector('#continuePendingSchedule').onclick=()=>openNewAppointmentForm();document.querySelector('#backPendingSchedule').onclick=()=>{state.draftAppointment.debtChecked=false;openNewAppointmentForm();};return;}}
...
function openClient(id){
  ...
  state.modal=`<div class="modal-head"><div><span class="eyebrow">CLIENTE</span><h2>${escapeHtml(c.name)}</h2></div><button data-close-modal>×</button></div><div class="tab-grid"><article><span>Histórico</span><strong>${aps.length}</strong></article><article class="received-summary"><span>Recebido</span><strong>${brl(cashPays.reduce((s,p)=>s+Number(p.amount),0))}</strong></article></div>${cycle?`<div class="cycle-alert"><b>${cycle} ${cycle===1?'manutenção':'manutenções'} desde a última aplicação</b>${cycle>=2?'<span>Avaliar nova aplicação no próximo retorno.</span>':''}</div>`:''}${c.walletEnabled?`<div class="cycle-alert"><b>Saldo em carteira: ${brl(wallet)}</b><span>Créditos de troca de serviços.</span></div><button class="secondary full" id="clientWallet">Carteira da cliente</button>`:''}<button class="secondary full" id="editClient">Editar cadastro</button><button class="primary full" id="clientSchedule">AGENDAR</button><button class="secondary full" id="clientSendRules">Enviar regras de agendamento</button><h3 class="history-title">Histórico de atendimentos</h3><div class="simple-list">${aps.length?aps.map(a=>{const paid=paymentTotalForAppointment(a.id);return `<div class="client-history-wrap"><button class="client-history history-columns" data-appointment="${a.id}"><span class="history-date">${dateWithWeekday(a.start)}<small>${fmtTime(a.start)}–${fmtTime(a.end)}</small></span><span class="history-proc">${escapeHtml(a.procedureNames?.join(', ')||'')}</span><b class="history-money">${brl(paid)} <small>de ${brl(a.totalValue)}</small>${Number(a.transferredOut||0)>0?`<small class="transferred-balance-note" style="display:block;white-space:nowrap;margin-top:3px;">Saldo transferido</small>`:''}</b></button>${a.status==='scheduled'?`<button class="history-conclude" data-conclude-client="${a.id}">Concluir</button>`:''}</div>`}).join(''):'<div class="empty">Sem atendimentos.</div>'}</div>`;render();
  document.querySelector('#clientSchedule').onclick=()=>openClientScheduleChoice(id);document.querySelector('#editClient').onclick=()=>editClient(id);document.querySelector('#clientSendRules').onclick=()=>shareBookingRulesImage();document.querySelector('#clientWallet')?.addEventListener('click',()=>openClientWallet(id));document.querySelector('#enableClientWallet')?.addEventListener('click',()=>{confirmDialog('Ativar saldo em carteira para esta cliente?',()=>{c.walletEnabled=true;c.walletTransactions=c.walletTransactions||[];store.save('client.wallet.enable',{clientId:id});openClient(id);});});document.querySelectorAll('[data-appointment]').forEach(b=>b.onclick=()=>openAppointment(b.dataset.appointment));document.querySelectorAll('[data-conclude-client]').forEach(b=>b.onclick=()=>completeAppointment(b.dataset.concludeClient));
}
...
function openAppointment(id){
  ...
  state.modal=`<div class="modal-head"><div><span class="eyebrow">AGENDAMENTO${completed?' • CONCLUÍDO':cancelled?' • CANCELADO':awaiting?' • AGUARDANDO CONFIRMAÇÃO':''}</span><h2>${escapeHtml(c?.name||'Cliente')}</h2></div><button data-close-modal>×</button></div><div class="detail-grid"><span>Data</span><b>${dateWithWeekday(a.start)}</b><span>Horário</span><b>${fmtTime(a.start)}–${fmtTime(a.end)}</b><span>Procedimentos</span><b>${escapeHtml(a.procedureNames?.join(', ')||'')}</b><span>Valor</span><b>${brl(a.totalValue)}</b>${a.carryOver?.total?`<span>Inclui saldo anterior</span><b>${brl(a.carryOver.total)}</b>`:''}${a.transferredOut?`<span>Saldo transferido</span><b>${brl(a.transferredOut)}</b>`:''}<span class="received-band received-pending">Recebido</span><b class="received-band received-pending">${brl(paid)}</b><span>A receber</span><b>${brl(balance)}</b></div>${a.notes?`<div class="note-box">${escapeHtml(a.notes)}</div>`:''}${awaiting?`<button class="primary full" id="reservationActions">Reserva / sinal</button><button class="secondary full" id="remindConfirmation">${messageButtonLabel(a,'confirmation_reminder')} • lembrete de confirmação</button>`:''}${!cancelled?`${financeButton}${!completed&&!awaiting?'<button class="success-action full" id="completeAppointment">✓ Concluir atendimento</button>':awaiting?'<div class="blocked-action"><b>Concluir atendimento bloqueado</b><span>Confirme o horário primeiro, com ou sem sinal.</span></div>':''}${completed&&balance>0?`<button class="primary full" id="payCompletedBalance">Finalizar recebimento • ${brl(balance)}</button>`:''}${completed?'<button class="secondary full" id="scheduleMaintenance">Agendar próximo atendimento / manutenção</button>':''}${completed&&care?'<button class="secondary full" id="sendPostCare">Enviar cuidados pós-atendimento</button>':''}${!completed?'<div class="action-pair"><button class="secondary" id="editSchedule">Alterar data / horário</button><button class="secondary" id="rescheduleCancel">Reagendar / cancelar</button></div>':''}${paid<=0&&!completed?'<button class="danger full" id="deleteAppointment">Excluir agendamento</button>':''}`:''}${a.history?.length?`<details class="appointment-history"><summary>Ver histórico do agendamento</summary>${a.history.slice().reverse().map(h=>`<div><b>${dateWithWeekday(h.at,true)}</b><span>${escapeHtml(friendlyHistoryLabel(h))}</span></div>`).join('')}</details>`:''}`;render();
...
function registerPayment(appointmentId,skipOtherPendingWarning=false){
  const a=currentData().appointments.find(x=>x.id===appointmentId),paid=paymentTotalForAppointment(appointmentId),balance=appointmentBalance(a);if(balance<=0){alert('Este atendimento já está quitado.');return;}
  if(!skipOtherPendingWarning){
    const otherPending=clientPendingAppointments(a.clientId,a.id);
    if(otherPending.length){
      const otherTotal=otherPending.reduce((sum,item)=>sum+appointmentBalance(item),0);
      state.modal=`<div class="modal-head"><div><span class="eyebrow">SALDO PENDENTE</span><h2>Há outros valores a receber</h2></div><button data-close-modal>×</button></div><div class="warning">Esta cliente possui <b>${brl(otherTotal)}</b> pendentes em outro(s) atendimento(s).</div><p class="muted">O valor abaixo não inclui saldos que já foram transferidos. Você pode continuar neste recebimento ou abrir primeiro uma das pendências.</p><button class="primary full" id="continueCurrentPayment">Continuar neste atendimento</button><button class="secondary full" id="openOtherPendingPayment">Ver primeira pendência • ${brl(appointmentBalance(otherPending[0]))}</button>`;
      render();
      document.querySelector('#continueCurrentPayment').onclick=()=>registerPayment(appointmentId,true);
      document.querySelector('#openOtherPendingPayment').onclick=()=>registerPayment(otherPending[0].id,true);
      return;
    }
  }
  const first=paid<=0,parts=paymentSuggestionParts(a);
  // Em atendimento concluído, o objetivo é quitar o saldo real. O sinal de 30% só é sugestão antes da conclusão.
  const suggested=a.status==='completed'?balance:parts.suggested;
  const signalLabel=parts.carry>0?'Sinal do novo serviço (30%)':'Sinal sugerido (30%)';
  const carryRows=parts.carry>0?`<span>Saldo pendente anterior</span><b>${brl(parts.carry)}</b><span>Novo serviço</span><b>${brl(parts.service)}</b>`:'';
  state.modal=`<div class="modal-head"><div><span class="eyebrow">FINANCEIRO</span><h2>Registrar pagamento</h2></div><button data-close-modal>×</button></div><div class="payment-summary payment-summary-unified"><span>Valor do atendimento</span><b>${brl(a.totalValue)}</b><span>${signalLabel}</span><b>${brl(parts.signal)}</b><span class="received-band received-pending">Já recebido</span><b class="received-band received-pending">${brl(paid)}</b><span>A receber</span><b>${brl(balance)}</b>${parts.carry>0?`<span>Saldo pendente anterior</span><b>${brl(parts.carry)}</b><span>Novo serviço</span><b>${brl(parts.service)}</b><span>Valor sugerido para lançar</span><b>${brl(suggested)}</b>`:''}</div>${currentData().clients.find(c=>c.id===a.clientId)?.walletEnabled&&walletBalance(currentData().clients.find(c=>c.id===a.clientId))>0?`<button class="secondary full" id="useWalletPayment">Usar saldo em carteira — ${brl(Math.min(walletBalance(currentData().clients.find(c=>c.id===a.clientId)),balance))} disponível</button>`:''}<label>Valor para lançar<input id="payAmount" type="number" step="0.01" min="0.01" max="${balance}" value="${suggested.toFixed(2)}"></label><label>Forma<select id="payMethod"><option>PIX</option><option>Dinheiro</option><option>Cartão</option></select></label><div class="live-payment-result"><span>Após este lançamento</span><strong id="paymentResult"></strong></div><button class="primary full" id="savePayment">LANÇAR</button>`;
  render();document.querySelector('#useWalletPayment')?.addEventListener('click',()=>useWalletForAppointment(appointmentId,'payment'));const input=document.querySelector('#payAmount');const refresh=()=>{const amount=Math.max(0,Number(input.value||0));document.querySelector('#paymentResult').textContent=`Recebido ${brl(paid+amount)} • A receber ${brl(Math.max(0,balance-amount))}`};input.oninput=refresh;refresh();document.querySelector('#savePayment').onclick=()=>{const amount=Number(input.value||0),method=document.querySelector('#payMethod').value;if(amount<=0||amount>balance){alert(`Informe um valor entre R$ 0,01 e ${brl(balance)}.`);return;}const kind=first&&a.status!=='completed'&&Math.abs(amount-parts.suggested)<.01?'Sinal / Entrada':amount>=balance?'Pagamento integral':'Pagamento parcial';confirmMoneyLaunch({amount,method,kind,onConfirm:()=>savePaymentRecord(a,amount,method,kind),onCancel:()=>registerPayment(a.id)});};
}
...
function editProcedure(id){
  ...
  document.querySelector('#saveProcedure').onclick=()=>{const valueEl=document.querySelector('#procValue'),hoursEl=document.querySelector('#procHours'),minutesEl=document.querySelector('#procMinutes'),maintenanceEl=document.querySelector('#procRequiresMaintenance'),descriptionEl=document.querySelector('#procDescription');if(!valueEl||!hoursEl||!minutesEl||!maintenanceEl||!descriptionEl){alert('Não foi possível ler os campos do procedimento. Feche esta tela e tente novamente.');return;}const draft={value:Number(valueEl.value),hours:Math.max(0,Number(hoursEl.value||0)),minutes:Math.min(59,Math.max(0,Number(minutesEl.value||0))),requiresMaintenance:maintenanceEl.checked,description:descriptionEl.value.trim(),image:newImage};if(!Number.isFinite(draft.value)||draft.value<0){alert('Informe um valor válido para o procedimento.');return;}confirmDialog('Salvar alterações deste procedimento?',()=>{p.value=draft.value;p.durationMin=draft.hours*60+draft.minutes;p.requiresMaintenance=draft.requiresMaintenance;p.description=draft.description;p.image=draft.image;store.save('procedure.update',{procedureId:id});state.modal=null;render();},()=>editProcedure(id));};
}
function editClient(id){const c=currentData().clients.find(x=>x.id===id);if(!c)return;state.modal=`<div class="modal-head"><div><span class="eyebrow">CLIENTE</span><h2>Editar cadastro</h2></div><button data-close-modal>×</button></div><label>Nome<input id="editClientName" value="${escapeHtml(c.name||'')}"></label><label>WhatsApp<input id="editClientWhatsapp" value="${escapeHtml(c.whatsapp||'')}"></label><label>Observações<textarea id="editClientNotes">${escapeHtml(c.notes||'')}</textarea></label><label class="check-row"><input id="editClientWallet" type="checkbox" ${c.walletEnabled?'checked':''}><span><b>Ativar carteira</b><small>Permite usar créditos de troca de serviços.</small></span></label><button class="primary full" id="saveClientEdit">Salvar alterações</button>`;render();document.querySelector('#saveClientEdit').onclick=()=>{const name=document.querySelector('#editClientName').value.trim();if(!name){alert('Informe o nome.');return;}c.name=name;c.whatsapp=document.querySelector('#editClientWhatsapp').value.trim();c.notes=document.querySelector('#editClientNotes').value.trim();c.walletEnabled=document.querySelector('#editClientWallet').checked;c.walletTransactions=c.walletTransactions||[];store.save('client.update',{clientId:id,walletEnabled:c.walletEnabled});openClient(id);};}
function recalcWalletBalances(c){let bal=0;walletTransactions(c).sort((a,b)=>new Date(a.at)-new Date(b.at)).forEach(t=>{bal+=Number(t.amount||0);t.balanceAfter=bal;});}
function editWalletTransaction(clientId,txId){const c=currentData().clients.find(x=>x.id===clientId),t=walletTransactions(c).find(x=>x.id===txId);if(!t)return;state.modal=`<div class="modal-head"><div><span class="eyebrow">CARTEIRA</span><h2>Editar valores</h2></div><button data-close-modal>×</button></div><label>Valor<input id="editWalletValue" type="number" step="0.01" min="0.01" value="${Math.abs(Number(t.amount||0)).toFixed(2)}"></label><label>Descrição<input id="editWalletDesc" value="${escapeHtml(t.description||'')}"></label><label>Data<input id="editWalletDate" type="date" value="${toLocalDateKey(new Date(t.at))}"></label><div class="warning">A alteração recalcula o saldo da carteira e preserva o histórico da movimentação.</div><button class="primary full" id="saveWalletEdit">Salvar alteração</button><button class="secondary full" id="backWalletEdit">Voltar</button>`;render();document.querySelector('#backWalletEdit').onclick=()=>openClientWallet(clientId);document.querySelector('#saveWalletEdit').onclick=()=>{const v=Number(document.querySelector('#editWalletValue').value||0);if(v<=0){alert('Informe um valor válido.');return;}const sign=Number(t.amount)>=0?1:-1;t.amount=sign*v;t.financeValue=v;t.description=document.querySelector('#editWalletDesc').value.trim()||t.description;t.at=new Date(`${document.querySelector('#editWalletDate').value}T12:00:00`).toISOString();recalcWalletBalances(c);store.save('client.wallet.edit',{clientId,transactionId:txId,amount:t.amount});openClientWallet(clientId);};}
function openExpenseReport(){const {start,end,label}=financeReportRange(),data=currentData(),expenses=data.expenses.filter(x=>new Date(x.date)>=start&&new Date(x.date)<=end).map(x=>({date:new Date(x.date+'T12:00:00'),desc:x.description||'Gasto com produto',value:Number(x.value||0)})),wallet=walletFinanceTransactionsInRange(start,end).filter(t=>t.financeEffect==='expense').map(t=>({date:new Date(t.at),desc:`Carteira • ${t.description||'movimentação'}`,value:Number(t.financeValue??Math.abs(t.amount))})),rows=[...expenses,...wallet].sort((a,b)=>a.date-b.date);state.modal=`<div class="modal-head"><div><span class="eyebrow">SAÍDAS</span><h2>${escapeHtml(label)}</h2></div><button data-close-modal>×</button></div><div class="received-total">Total: <b>${brl(rows.reduce((s,x)=>s+x.value,0))}</b></div><div class="simple-list">${rows.length?rows.map(x=>`<div class="receipt-row"><div><strong>${escapeHtml(x.desc)}</strong><span>${dateWithWeekday(x.date)}</span></div><b>${brl(x.value)}</b></div>`).join(''):'<div class="empty">Nenhuma saída neste período.</div>'}</div>`;render();}
function openAppointmentsReport(){const {start,end,label}=financeReportRange(),rows=currentData().appointments.filter(a=>a.type!=='personal'&&a.status==='completed'&&new Date(a.start)>=start&&new Date(a.start)<=end).sort((a,b)=>new Date(a.start)-new Date(b.start));state.modal=`<div class="modal-head"><div><span class="eyebrow">ATENDIMENTOS</span><h2>${escapeHtml(label)}</h2></div><button data-close-modal>×</button></div><div class="received-total">Realizados: <b>${rows.length}</b></div><div class="simple-list">${rows.length?rows.map(a=>{const c=currentData().clients.find(x=>x.id===a.clientId);return `<button class="receipt-row" data-report-appt="${a.id}"><div><strong>${escapeHtml(c?.name||'Cliente')}</strong><span>${dateWithWeekday(a.start)} • ${fmtTime(a.start)}–${fmtTime(a.end)} • ${escapeHtml(a.procedureNames?.join(', ')||'')}</span></div><b>${brl(a.totalValue)}</b></button>`}).join(''):'<div class="empty">Nenhum atendimento realizado neste período.</div>'}</div>`;render();document.querySelectorAll('[data-report-appt]').forEach(b=>b.onclick=()=>openAppointment(b.dataset.reportAppt));}
...
function openReceivedLedger(){const start=startOfWeek(state.selectedWeek),end=endOfWeek(state.selectedWeek),pays=currentData().payments.filter(p=>new Date(p.at)>=start&&new Date(p.at)<=end&&!isWalletPayment(p)).sort((a,b)=>new Date(a.at)-new Date(b.at));state.modal=`<div class="modal-head"><div><span class="eyebrow">RECEBIDO</span><h2>Extrato da semana</h2></div><button data-close-modal>×</button></div><div class="received-total">Total: <b>${brl(pays.reduce((s,p)=>s+Number(p.amount),0))}</b></div><div class="simple-list">${pays.length?pays.map(p=>{const a=currentData().appointments.find(x=>x.id===p.appointmentId),c=currentData().clients.find(x=>x.id===a?.clientId);return `<button class="receipt-row" data-payment-edit="${p.id}"><div><strong>${escapeHtml(c?.name||'Cliente')}</strong><span>${escapeHtml(a?.procedureNames?.join(', ')||'Atendimento')} • ${dateWithWeekday(p.at)} • toque para editar</span></div><b>${brl(p.amount)}</b></button>`}).join(''):'<div class="empty">Nenhum recebimento nesta semana.</div>'}</div>`;render();document.querySelectorAll('[data-payment-edit]').forEach(b=>b.onclick=()=>editPaymentRecord(b.dataset.paymentEdit));}
...
function openCompletionReview(id){const a=currentData().appointments.find(x=>x.id===id);if(!a)return;const names=(a.procedureNames||[]).join(', ')||'Atendimento',defaultMaint=appointmentRequiresMaintenance(a)||(a.procedureIds||[]).includes('alongamento');state.modal=`<div class="modal-head"><div><span class="eyebrow">CONCLUIR ATENDIMENTO</span><h2>Conferir o que foi realizado</h2></div><button data-close-modal>×</button></div><div class="cycle-alert"><b>${escapeHtml(names)} lançado automaticamente</b><span>Valor atual do procedimento: ${brl(a.totalValue)}</span></div><label class="check-row maintenance-setting"><input id="completionMaintenance" type="checkbox" ${defaultMaint?'checked':''}><span><b>Este atendimento terá manutenção / retorno</b><small>Alongamento já vem marcado automaticamente.</small></span></label><div class="warning"><b>Foi realizado algum procedimento além do que já estava agendado?</b></div><button class="primary full" id="addExtraProcedure">Sim, adicionar outro procedimento</button><button class="secondary full" id="finishAsBooked">Não, concluir como está</button>`;render();const finish=()=>{a.requiresMaintenance=document.querySelector('#completionMaintenance')?.checked??defaultMaint;const pending=pendingBalanceSummary(a.clientId,a.id,a.start);const conclude=(transfer)=>{if(transfer&&pending.total>0)carryPendingIntoAppointment(a,pending.items);a.status='completed';a.completedAt=new Date().toISOString();addHistory(a,'completed','Atendimento concluído');store.save('appointment.complete',{appointmentId:id,total:a.totalValue,requiresMaintenance:a.requiresMaintenance,carriedPreviousBalance:transfer?pending.total:0});openCompletionNext(id);};if(pending.total>0){state.modal=`<div class="modal-head"><div><span class="eyebrow">SALDO PENDENTE ANTERIOR</span><h2>${brl(pending.total)} a receber</h2></div><button data-close-modal>×</button></div><div class="warning">Esta cliente possui <b>${brl(pending.total)}</b> pendentes de atendimento anterior.</div><p class="muted">Deseja adicionar esse saldo ao valor deste atendimento? O histórico anterior ficará marcado como transferido, evitando cobrança duplicada.</p><button class="primary full" id="carryDebtOnComplete">Adicionar ${brl(pending.total)} e concluir</button><button class="secondary full" id="completeWithoutDebt">Concluir sem adicionar</button>`;render();document.querySelector('#carryDebtOnComplete').onclick=()=>conclude(true);document.querySelector('#completeWithoutDebt').onclick=()=>conclude(false);return;}conclude(false);};document.querySelector('#finishAsBooked').onclick=finish;document.querySelector('#addExtraProcedure').onclick=()=>openExtraProcedureReview(id,document.querySelector('#completionMaintenance')?.checked??defaultMaint);}
...
function startMaintenanceFromCompletion(id){const a=currentData().appointments.find(x=>x.id===id);if(!a)return;state.returnAppointmentId=id;startAppointmentFlow(a.clientId,'new');}

function openClientWallet(clientId){const c=currentData().clients.find(x=>x.id===clientId);if(!c)return;const tx=walletTransactions(c).slice().sort((a,b)=>new Date(b.at)-new Date(a.at));state.modal=`<div class="modal-head"><div><span class="eyebrow">CARTEIRA DA CLIENTE</span><h2>${escapeHtml(c.name)}</h2></div><button data-close-modal>×</button></div><div class="cycle-alert"><b>Saldo atual: ${brl(walletBalance(c))}</b></div><button class="primary full" id="addWalletCredit">+ Adicionar saldo</button><button class="secondary full" id="toggleWalletOff">Desativar saldo em carteira</button><div class="simple-list">${tx.length?tx.map(t=>`<div class="receipt-row"><div><strong>${escapeHtml(t.description||'Movimentação')}</strong><span>${dateWithWeekday(t.at)}${t.appointmentId?' • atendimento':''}</span></div><b>${t.amount>=0?'+ ': '− '}${brl(Math.abs(t.amount))}<small> • saldo ${brl(t.balanceAfter??0)}</small><button class="wallet-edit-mini" data-wallet-edit="${t.id}">Editar valores</button></b></div>`).join(''):'<div class="empty">Nenhuma movimentação na carteira.</div>'}</div>`;render();document.querySelector('#addWalletCredit').onclick=()=>openWalletCreditForm(clientId);document.querySelectorAll('[data-wallet-edit]').forEach(b=>b.onclick=()=>editWalletTransaction(clientId,b.dataset.walletEdit));document.querySelector('#toggleWalletOff').onclick=()=>{confirmDialog('Desativar saldo em carteira? O saldo e todo o histórico serão preservados.',()=>{c.walletEnabled=false;store.save('client.wallet.disable',{clientId});openClient(clientId);});};}
...
function useWalletForAppointment(appointmentId,origin='completion'){const a=currentData().appointments.find(x=>x.id===appointmentId),c=currentData().clients.find(x=>x.id===a?.clientId);if(!a||!c)return;const available=walletBalance(c),balance=appointmentBalance(a),amount=Math.min(available,balance);if(amount<=0){alert('Não há saldo disponível na carteira.');return;}state.modal=`<div class="modal-head"><div><span class="eyebrow">SALDO EM CARTEIRA</span><h2>Usar ${brl(amount)}</h2></div><button data-close-modal>×</button></div><div class="payment-summary"><span>Saldo disponível</span><b>${brl(available)}</b><span>Abatimento</span><b>${brl(amount)}</b><span>Saldo após uso</span><b>${brl(available-amount)}</b></div><label>Refletir este abatimento no Financeiro?<select id="walletDebitFinance"><option value="none">Não lançar no Financeiro</option><option value="income">Sim — como Entrada</option><option value="expense">Sim — como Saída</option></select></label><button class="primary full" id="confirmWalletDebit">Confirmar uso da carteira</button><button class="secondary full" id="cancelWalletDebit">Cancelar</button>`;render();document.querySelector('#cancelWalletDebit').onclick=()=>origin==='payment'?registerPayment(appointmentId):openCompletionNext(appointmentId);document.querySelector('#confirmWalletDebit').onclick=()=>{const effect=document.querySelector('#walletDebitFinance').value,before=available,at=new Date().toISOString();walletTransactions(c).push({id:uid('wallet'),amount:-amount,description:`${a.procedureNames?.join(', ')||'Atendimento'}`,at,appointmentId:a.id,balanceAfter:before-amount,financeEffect:effect,financeValue:amount});currentData().payments.push({id:uid('pay'),appointmentId:a.id,amount,method:'Saldo em carteira',kind:'wallet',at});addHistory(a,'wallet_payment',`Pagamento com saldo em carteira: ${brl(amount)}${effect!=='none'?` • financeiro: ${effect==='income'?'entrada':'saída'}`:''}`,{amount,financeEffect:effect});store.save('client.wallet.debit',{clientId:c.id,appointmentId:a.id,amount,financeEffect:effect});if(appointmentBalance(a)>0)registerPayment(a.id);else if(origin==='payment')openAppointment(a.id);else openCompletionNext(a.id);};}
