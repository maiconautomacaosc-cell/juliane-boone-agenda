...
function appointmentRow(a) {
  ...
  const status=a.status==='awaiting_confirmation'?'Aguardando':a.status==='completed'?(paid>=Number(a.totalValue||0)&&a.totalValue>0?'Realizado • Pago':paid>0?'Realizado • Parcial':'Realizado • A receber'):paid>=Number(a.totalValue||0)&&a.totalValue>0?'Confirmado • Pago':paid>0?'Confirmado • Parcial':'Confirmado';
...
function finance() {
  ...
  return `<main class="content"><section class="hero-card"><span class="eyebrow">FINANCEIRO</span><h1>Entradas e gastos</h1><p>${weekLabel(state.selectedWeek)}</p><div class="week-controls finance-week"><button id="prevWeek">‹</button><button id="thisWeek">Esta semana</button><button id="nextWeek">›</button></div></section>
    ...
    <section class="section-card"><div class="section-head"><div><span class="eyebrow">GASTOS COM PRODUTOS</span><h2>Lançamentos</h2></div><button class="primary small" id="addExpense">+ Gasto</button></div><div class="simple-list">${exps.length?exps.slice().reverse().map(x=>`<button class="simple-row expense-edit-row" data-expense-edit="${x.id}"><div><strong>${escapeHtml(x.description)}</strong><span>${new Date(x.date+'T12:00:00').toLocaleDateString('pt-BR')} • toque para editar</span></div><b>${brl(x.value)}</b></button>`).join(''):'<div class="empty">Nenhum gasto nesta semana.</div>'}</div></section></main>`;
}
...
function openAppointment(id){
  ...
  state.modal=`<div class="modal-head"><div><span class="eyebrow">AGENDAMENTO${completed?' • CONCLUÍDO':cancelled?' • CANCELADO':awaiting?' • AGUARDANDO CONFIRMAÇÃO':''}</span><h2>${escapeHtml(c?.name||'Cliente')}</h2></div><button data-close-modal>×</button></div><div class="detail-grid"><span>Data</span><b>${new Date(a.start).toLocaleDateString('pt-BR')}</b><span>Horário</span><b>${fmtTime(a.start)}–${fmtTime(a.end)}</b><span>Procedimentos</span><b>${escapeHtml(a.procedureNames?.join(', ')||'')}</b><span>Valor</span><b>${brl(a.totalValue)}</b><span class="received-band ${balance<=0?'received-paid':'received-pending'}">Recebido</span><b class="received-band ${balance<=0?'received-paid':'received-pending'}">${brl(paid)}</b><span>A receber</span><b>${brl(balance)}</b></div>${a.notes?`<div class="note-box">${escapeHtml(a.notes)}</div>`:''}${awaiting?`<button class="primary full" id="reservationActions">Reserva / sinal</button><button class="secondary full" id="remindConfirmation">${messageButtonLabel(a,'confirmation_reminder')} • lembrete de confirmação</button>`:''}${!cancelled?`${financeButton}${!completed?'<button class="success-action full" id="completeAppointment">✓ Concluir atendimento</button>':''}${completed&&needsMaintenance?'<button class="secondary full" id="scheduleMaintenance">Agendar próxima manutenção</button>':''}${completed&&care?'<button class="secondary full" id="sendPostCare">Enviar cuidados pós-atendimento</button>':''}${!completed?'<div class="action-pair"><button class="secondary" id="editSchedule">Alterar data / horário</button><button class="secondary" id="rescheduleCancel">Reagendar / cancelar</button></div>':''}${paid<=0&&!completed?'<button class="danger full" id="deleteAppointment">Excluir agendamento</button>':''}`:''}${a.history?.length?`<details class="appointment-history"><summary>Ver histórico do agendamento</summary>${a.history.slice().reverse().map(h=>`<div><b>${new Date(h.at).toLocaleString('pt-BR')}</b><span>${escapeHtml(friendlyHistoryLabel(h))}</span></div>`).join('')}</details>`:''}`;render();
...
function savePaymentRecord(a,amount,method,kind){
  ...
  const newPaid=paymentTotalForAppointment(a.id),client=currentData().clients.find(c=>c.id===a.clientId),balance=Math.max(0,a.totalValue-newPaid);
  if(wasCompleted){
    a.status='completed';
...
function openScheduleTimeEditor(id,key,kind='alteracao',prefillTime=null){const a=currentData().appointments.find(x=>x.id===id);if(!a)return;const originalMinutes=Math.max(15,Math.round((new Date(a.end)-new Date(a.start))/60000)),st=prefillTime||fmtTime(a.start),base=new Date(`${key}T${st}:00`),autoEnd=addMinutes(base,originalMinutes),et=`${String(autoEnd.getHours()).padStart(2,'0')}:${String(autoEnd.getMinutes()).padStart(2,'0')}`;state.modal=`<div class="modal-head"><div><span class="eyebrow">${kind==='reagendamento'?'REAGENDAR':'ALTERAR AGENDAMENTO'}</span><h2>${new Date(key+'T12:00:00').toLocaleDateString('pt-BR')}</h2></div><button data-close-modal>×</button></div><p class="muted">Data escolhida pela sua Agenda. Agora confirme o horário.</p><div class="two-col"><label>Início<input id="editStart" type="time" value="${st}"></label><label>Fim<input id="editEnd" type="time" value="${et}"></label></div><button class="primary full" id="saveScheduleEdit">Salvar ${kind==='reagendamento'?'reagendamento':'alteração'}</button>`;render();document.querySelector('#saveScheduleEdit').onclick=()=>{const old=`${new Date(a.start).toLocaleDateString('pt-BR')} ${fmtTime(a.start)}–${fmtTime(a.end)}`,sv=document.querySelector('#editStart').value,ev=document.querySelector('#editEnd').value,sn=new Date(`${key}T${sv}:00`),en=new Date(`${key}T${ev}:00`);if(en<=sn)en.setDate(en.getDate()+1);const candidate={...a,start:sn.toISOString(),end:en.toISOString()},conflict=hasConflict(candidate,a.id);if(conflict){alert(`Horário indisponível: já existe compromisso das ${fmtTime(conflict.start)} às ${fmtTime(conflict.end)}.`);return;}a.start=candidate.start;a.end=candidate.end;addHistory(a,kind==='reagendamento'?'rescheduled':'schedule_change',`${kind==='reagendamento'?'Reagendado':'Data/horário alterado'}: ${old} → ${new Date(a.start).toLocaleDateString('pt-BR')} ${fmtTime(a.start)}–${fmtTime(a.end)}`);store.save('appointment.schedule.update',{appointmentId:id,kind});openAppointment(id);};}
...
function openCancelFinancial(id){const a=currentData().appointments.find(x=>x.id===id),paid=Math.max(0,paymentTotalForAppointment(id));state.modal=`<div class="modal-head"><div><span class="eyebrow">CANCELAMENTO • FINANCEIRO</span><h2>Destino do valor recebido</h2></div><button data-close-modal>×</button></div><div class="payment-summary"><span>Total recebido</span><b>${brl(paid)}</b></div><p class="muted">Os valores começam preenchidos automaticamente. Edite se houver negociação parcial.</p><label>Valor devolvido<input type="number" id="cancelRefund" min="0" max="${paid}" step="0.01" value="${paid.toFixed(2)}"></label><label>Valor retido<input type="number" id="cancelRetained" min="0" max="${paid}" step="0.01" value="0.00"></label><div class="warning" id="cancelMoneyInfo">Devolvido + retido deve ser exatamente ${brl(paid)} e nunca pode ultrapassar o recebido.</div><button class="danger full" id="confirmCancelFinancial">Confirmar cancelamento</button><button class="secondary full" id="backCancelFinancial">Voltar</button>`;render();const refund=document.querySelector('#cancelRefund'),retained=document.querySelector('#cancelRetained');let syncing=false;const sync=(source)=>{if(syncing)return;syncing=true;let v=Math.max(0,Math.min(paid,Number(source.value||0)));source.value=v.toFixed(2);const other=source===refund?retained:refund;other.value=Math.max(0,paid-v).toFixed(2);syncing=false;};refund.oninput=()=>sync(refund);retained.oninput=()=>sync(retained);document.querySelector('#backCancelFinancial').onclick=()=>rescheduleOrCancel(id);document.querySelector('#confirmCancelFinancial').onclick=()=>{const r=Number(refund.value||0),t=Number(retained.value||0);if(r<0||t<0||r>paid||t>paid||r+t>paid+0.005){alert('O valor devolvido + retido não pode ultrapassar o total recebido.');return;}if(Math.abs((r+t)-paid)>0.01){alert(`Distribua todo o valor recebido (${brl(paid)}) entre devolvido e retido.`);return;}if(!confirm(`Confirmar cancelamento?\nDevolvido: ${brl(r)}\nRetido: ${brl(t)}`))return;finishCancellation(id,r,t);};}
...
async function shareRulesAndText(a){
  const text=reservationText(a,false);
  try{
    const response=await fetch(BOOKING_RULES_IMAGE,{cache:'no-store'});
    if(!response.ok)throw new Error('image');
    const blob=await response.blob();
    const file=new File([blob],'regras-agendamento-juliane-boone.jpg',{type:blob.type||'image/jpeg'});
    if(navigator.share&&(!navigator.canShare||navigator.canShare({files:[file]}))){
      await navigator.share({files:[file],text,title:'Regras + sinal • Juliane Boone'});
      markMessageSent(a,'reservation_full');
      return;
    }
  }catch(err){if(err?.name==='AbortError')return;}
  const c=currentData().clients.find(x=>x.id===a.clientId);
...
function openReceivedLedger(){const start=startOfWeek(state.selectedWeek),end=endOfWeek(state.selectedWeek),pays=currentData().payments.filter(p=>new Date(p.at)>=start&&new Date(p.at)<=end).sort((a,b)=>new Date(b.at)-new Date(a.at));state.modal=`<div class="modal-head"><div><span class="eyebrow">RECEBIDO</span><h2>Extrato da semana</h2></div><button data-close-modal>×</button></div><div class="received-total">Total: <b>${brl(pays.reduce((s,p)=>s+Number(p.amount),0))}</b></div><div class="simple-list">${pays.length?pays.map(p=>{const a=currentData().appointments.find(x=>x.id===p.appointmentId),c=currentData().clients.find(x=>x.id===a?.clientId);return `<button class="receipt-row" data-payment-edit="${p.id}"><div><strong>${escapeHtml(c?.name||'Cliente')}</strong><span>${escapeHtml(a?.procedureNames?.join(', ')||'Atendimento')} • ${new Date(p.at).toLocaleDateString('pt-BR')} • toque para editar</span></div><b>${brl(p.amount)}</b></button>`}).join(''):'<div class="empty">Nenhum recebimento nesta semana.</div>'}</div>`;render();document.querySelectorAll('[data-payment-edit]').forEach(b=>b.onclick=()=>editPaymentRecord(b.dataset.paymentEdit));}
...
function openPersonalDetail(id){const a=currentData().appointments.find(x=>x.id===id);const done=a.status==='completed';state.modal=`<div class="modal-head personal-head"><div><span class="eyebrow">PARTICULAR${done?' • CONCLUÍDO':''}</span><h2>${escapeHtml(a.personalKind||'Compromisso')}</h2></div><button data-close-modal>×</button></div><div class="detail-grid"><span>Data</span><b>${new Date(a.start).toLocaleDateString('pt-BR')}</b><span>Horário</span><b>${a.allDay?'Dia inteiro':`${fmtTime(a.start)}–${fmtTime(a.end)}`}</b><span>Status</span><b>${done?'Concluído':'Em andamento'}</b></div>${a.notes?`<div class="note-box">${escapeHtml(a.notes)}</div>`:''}${!done?'<button class="success-action full" id="completePersonal">✓ Concluir compromisso</button>':''}<button class="secondary full" id="editPersonal">Alterar</button><button class="danger full" id="deletePersonal">Excluir</button>`;render();document.querySelector('#editPersonal').onclick=()=>openPersonalForm(id);document.querySelector('#completePersonal')?.addEventListener('click',()=>completePersonalFlow(id));document.querySelector('#deletePersonal').onclick=()=>{if(confirm('Excluir este compromisso particular?')){currentData().appointments=currentData().appointments.filter(x=>x.id!==id);store.save('personal.delete',{appointmentId:id});state.modal=null;render();}};}
...
function openExtraProcedureReview(id,requiresMaintenance){const a=currentData().appointments.find(x=>x.id===id),existingIds=new Set(a.procedureIds||[]),procs=currentData().procedures.filter(p=>p.active&&!existingIds.has(p.id));state.modal=`<div class="modal-head"><div><span class="eyebrow">CONCLUIR ATENDIMENTO</span><h2>Adicionar procedimento realizado</h2></div><button data-close-modal>×</button></div><div class="cycle-alert"><b>Já agendado: ${escapeHtml((a.procedureNames||[]).join(', '))}</b><span>Selecione abaixo somente o que foi feito a mais.</span></div><fieldset>${procs.map(p=>`<div class="procedure-choice"><label class="check-row"><input type="checkbox" class="finish-extra-proc" value="${p.id}"><span>${escapeHtml(p.name)}</span><b>${brl(p.value)}</b></label>${p.id==='reposicao-quebrada'?`<div class="qty-control"><button type="button" data-finish-minus>−</button><strong id="finishRepairQty">1</strong><button type="button" data-finish-plus>+</button><small>unidade(s)</small></div>`:''}</div>`).join('')}</fieldset><div class="completion-total">Total final <strong id="finishTotal">${brl(a.totalValue)}</strong></div><button class="primary full" id="confirmCompletion">Adicionar e concluir</button><button class="secondary full" id="backCompletion">Voltar</button>`;render();const calc=()=>{const qty=Number(document.querySelector('#finishRepairQty')?.textContent||1),items=[...document.querySelectorAll('.finish-extra-proc:checked')].map(x=>currentData().procedures.find(p=>p.id===x.value)),extra=items.reduce((sum,p)=>sum+p.value*(p.id==='reposicao-quebrada'?qty:1),0);document.querySelector('#finishTotal').textContent=brl(Number(a.totalValue)+extra);return {items,qty,extra};};document.querySelectorAll('.finish-extra-proc').forEach(x=>x.onchange=calc);document.querySelector('[data-finish-plus]')?.addEventListener('click',()=>{const q=document.querySelector('#finishRepairQty');q.textContent=Number(q.textContent)+1;calc();});document.querySelector('[data-finish-minus]')?.addEventListener('click',()=>{const q=document.querySelector('#finishRepairQty');q.textContent=Math.max(1,Number(q.textContent)-1);calc();});document.querySelector('#backCompletion').onclick=()=>openCompletionReview(id);document.querySelector('#confirmCompletion').onclick=()=>{const {items,qty,extra}=calc();if(!items.length){alert('Selecione o procedimento feito a mais ou volte e conclua como está.');return;}const extras=items.map(p=>({id:p.id,name:p.name,qty:p.id==='reposicao-quebrada'?qty:1,unitValue:p.value,value:p.value*(p.id==='reposicao-quebrada'?qty:1),durationMin:p.durationMin*(p.id==='reposicao-quebrada'?qty:1)}));a.procedureItems=[...(a.procedureItems||[]),...extras];a.procedureIds=[...(a.procedureIds||[]),...extras.map(x=>x.id)];a.procedureNames=[...(a.procedureNames||[]),...extras.map(i=>i.qty>1?`${i.name} ×${i.qty}`:i.name)];a.totalValue=Number(a.totalValue)+extra;a.requiresMaintenance=!!requiresMaintenance||extras.some(x=>procedureRequiresMaintenance(x.id));a.status='completed';a.completedAt=new Date().toISOString();addHistory(a,'completed',`Atendimento concluído • procedimento adicional: ${extras.map(x=>x.name).join(', ')}`);store.save('appointment.complete',{appointmentId:id,total:a.totalValue,requiresMaintenance:a.requiresMaintenance});openCompletionNext(id);};calc();}
...
function sendCompletionMessage(id){const a=currentData().appointments.find(x=>x.id===id),c=currentData().clients.find(x=>x.id===a.clientId),paid=paymentTotalForAppointment(id),balance=Math.max(0,a.totalValue-paid);const msg=`Olá, ${c?.name||''}! 😊\nSeu atendimento foi concluído. 💅\n\n💅 ${a.procedureNames?.join(', ')||'Atendimento'}\n💰 Valor total: ${brl(a.totalValue)}\n✅ Recebido: ${brl(paid)}${balance>0?`\n💰 Restante: ${brl(balance)}`:'\n💰 Pagamento QUITADO'}\n\nMuito obrigada pela preferência! 💕\nJuliane Boone Nail Designer`;openWhatsApp(c?.whatsapp,msg);markMessageSent(a,'completion_payment');openCompletionNext(id);}
function openCompletionNext(id){const a=currentData().appointments.find(x=>x.id===id),balance=Math.max(0,a.totalValue-paymentTotalForAppointment(id)),care=a.procedureIds.some(x=>x==='alongamento'||x==='manutencao'),needsMaintenance=appointmentRequiresMaintenance(a);state.modal=`<div class="modal-head"><div><span class="eyebrow">ATENDIMENTO CONCLUÍDO</span><h2>Próximos passos</h2></div><button data-close-modal>×</button></div>${balance>0?`<div class="warning">Saldo a receber: <b>${brl(balance)}</b></div><button class="primary full" id="finishFinance">Ir para Financeiro</button>`:`<div class="cycle-alert"><b>Pagamento quitado.</b></div><button class="primary full" id="finishCompletionMessage">${messageButtonLabel(a,'completion_payment')} • atendimento concluído</button>`}${care?`<button class="secondary full" id="finishCare">${messageButtonLabel(a,'post_care')} • cuidados pós-atendimento</button>`:''}${needsMaintenance?'<div class="cycle-alert"><b>Este atendimento possui manutenção.</b><span>Deseja programar o próximo retorno?</span></div><button class="secondary full" id="finishSchedule">Agendar próxima manutenção</button>':''}<button class="secondary full" id="finishClose">Finalizar por agora</button>`;render();document.querySelector('#finishFinance')?.addEventListener('click',()=>registerPayment(id));document.querySelector('#finishCompletionMessage')?.addEventListener('click',()=>sendCompletionMessage(id));document.querySelector('#finishCare')?.addEventListener('click',()=>sharePostCare(id));document.querySelector('#finishSchedule')?.addEventListener('click',()=>startMaintenanceFromCompletion(a.id));document.querySelector('#finishClose').onclick=()=>{state.modal=null;render();};}

async function sharePostCare(id){const a=currentData().appointments.find(x=>x.id===id),c=currentData().clients.find(x=>x.id===a.clientId),text=`Oi, ${c?.name||''}! 💅 Seguem os cuidados pós-atendimento para conservar seu alongamento/manutenção. Qualquer dúvida, estou à disposição. ❤️`;openWhatsApp(c?.whatsapp,text);markMessageSent(a,'post_care');setTimeout(()=>{if(confirm('A conversa da cliente foi aberta. Deseja abrir o PDF de cuidados para anexar no WhatsApp?'))window.open(POST_CARE_PDF,'_blank');},900);}
